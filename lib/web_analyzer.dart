...
class WebAnalyzer {
  ...
  static Future<Response?> _requestUrl(String url,
    ...
    final tempCookie = cookie ?? _cookies[uri.host];
    final request = Request('GET', uri)
      ...
      ..headers["accept"] = "*/*";
    if (tempCookie != null) {
      request.headers["Cookie"] = tempCookie;
    }
  ...
  static String _getHeadHtml(String html) {
    ...
    matches.forEach((element) {
      ...
      if (str?.contains(_titleReg) == true) head.writeln(str);
    });
  ...
  static String? _analyzeIcon(dom.Document document, Uri uri) {
    ...
    dom.Element? metaIcon = meta?.firstWhereOrNull<dom.Element>((e) {
      ...
      if (rel == "icon") {
        ...
        if (icon != null && icon?.toLowerCase().contains(".svg") == false) {
          return true;
        }
      }
      return false;
